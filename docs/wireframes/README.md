...
Open local HTML in a browser (self-contained). Regenerate gold report: `botdraw gold report`. Regenerate vision history: `botdraw vision loop-demo`.

**Live Dev Lab:** PortraitBot Phase B chrome is wired in `botdraw/web/` — calculation window (structure / ShadeField / assessment), gate checklist (Apply locked until all checks), defaults Ingest + classic + hatch off + underlay none.